...
public class LauncherFrame extends JFrame implements ActionListener, KeyListener, MouseWheelListener, DownloadListener, ISkinListener, IAuthListener {
    ...
    public LauncherFrame(SkinRepository skinRepo, UserModel userModel, AvailablePackList packList, MirrorStore mirrorStore) {
    ...
    private void initComponents(AvailablePackList packList) {
        ...
        launch = new BlueButton("PLAY");
        launch.setFont(getFont(24));
        ...
        platform = new ImageHyperlinkButton("http://loveplatform.kytech.it/");
        platform.setIcon(ResourceUtils.getIcon("openPlatformPage.png", 20, 20));
        platform.setRolloverIcon(ResourceUtils.getIcon("openPlatformPageInverted.png", 20, 20));
        ...
        customName.setFont(font.deriveFont(14F));
    ...
    public static Font getFont(int size) {
        Font minecraft;
        try {
            minecraft = Font.createFont(Font.TRUETYPE_FONT, getResourceAsStream("/org/spoutcraft/launcher/resources/Raleway-SemiBold.ttf")).deriveFont((float) size);
        } catch (Exception e) {
            e.printStackTrace();
            // Fallback
